<?php
	...
	if(isset($_FILES['image'])){
		...
		if($type == IMAGETYPE_JPEG || $type == IMAGETYPE_GIF || $type == IMAGETYPE_PNG){
			...
			$ratio = 300 / imagesx($image);
			$height = imagesy($image) * $ratio;
			$new_image = imagecreatetruecolor(300, $height);
			imagecopyresampled($new_image, $image, 0, 0, 0, 0, 300, $height, imagesx($image), imagesy($image));
			...
			$results = array();
			ExecutePreparedCallableStatement('CALL updateCategoryPicture(?,?)', 'is', $params, $results, function($results){});
