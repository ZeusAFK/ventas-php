<?php
	...
	if(isset($_FILES['image'])){
		...
		if($type == IMAGETYPE_JPEG || $type == IMAGETYPE_GIF || $type == IMAGETYPE_PNG){
			...
			$new_image = imagecreatetruecolor(930, 480);
			imagecopyresampled($new_image, $image, 0, 0, 0, 0, 930, 480, imagesx($image), imagesy($image));
			...
			$results = array();
			ExecutePreparedCallableStatement('CALL updateSliderElementPicture(?,?)', 'is', $params, $results, function($results){});
